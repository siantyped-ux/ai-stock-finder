...
3. 종목 수가 전체 스냅샷 중앙값의 80% 미만인 스냅샷은 손상으로 보고 제외한다.
   50%로는 걸러지지 않는다 — 손상본 773종목이 중앙값(1082)의 71%라 임계선
   위에 남는다. 80% 기준선 866은 손상본 773과 정상 최솟값 1061 사이의
   공백 한가운데에 놓여 양쪽 모두 여유가 크다.
...
### 소급분 스코어의 한계 (2026-08-18 발견)

구현 중 `fetch_stock`이 yfinance의 미확정 봉(OHLC가 NaN이고 거래량만 채워진
마지막 봉)을 걸러내지 않는 결함을 발견했다. 이 봉이 종가 배열에 남으면
`calc_tech_score`가 무너진다 — NVDA 실측으로 tech 72 -> 45, r3m -> NaN이었고
이동평균 정배열·MACD 판정이 통째로 사라졌다. tech는 총점 가중치 35%다.

결함은 `drop_unsettled_bars`로 수정했으나, **git 스냅샷에 이미 기록된 과거
스코어는 오염된 상태 그대로다.** 따라서 `source=backfill` 행의 스코어는
"그때 대시보드가 보여준 값"이지 "올바른 값"이 아니다. 영향은 미국 종목에
집중되며(한국 종목은 장 마감 후 스캔이라 미확정 봉이 없다), 스캔 시각과
데이터 확정 시점의 관계에 따라 날짜마다 다르게 나타난다.

백테스트 시 `source` 열로 반드시 구분할 것. 소급분은 시그널 성능 검증의
근거로 쓸 수 없고, 파이프라인 검증과 가격 이력 용도로만 유효하다.
정식 검증은 `source=live` 데이터가 쌓인 뒤에 시작해야 한다.
